...
TCP ist ein E2E Protokoll, das zusammen mit dem Internet Protokoll (IP) genutzt wird, um Daten in Form von Paketen zwischen Computern über das Internet zu übertragen. Während IP dafür sorgt, dass Daten tatsächlich zugestellt werden, kümmert sich TCP darum, die einzelnen Datenpakete nachzuverfolgen, in die Daten für ein effizientes Routen durch das Internet aufgeteilt werden. TCP implementiert einen Algorithmus zur Flusssteuerung, der "`Sliding Window"' genannt wird. Das "`Window"' ist die maximale Anzahl an Paketen, die gesendet werden können ohne auf Rückmeldung warten zu müssen. Der Algorithmus besteht aus folgenden Schritten \cite{Oldak2006}:
\begin{enumerate}
	...
	\item Wenn die Rückmeldung eines Pakets nicht in einer angegebenen Zeit erfolgt, dann wird das Paket neu verschickt
\end{enumerate}
